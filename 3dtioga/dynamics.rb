...
class MyPlots
  ...
  def ib_diagram
    ...
    t.show_plot([-1.0/@zoom, 1.0/@zoom, 1.0/@zoom, -1.0/@zoom]) do
      ...
      # draw the force
      #
      pt1 = project_point([-0.5 * (1.0+0.7), 0.0, 0.0])
      pt2 = project_point([-0.5 * (1.0+0.7), 0.0, 0.25])
      ...
      pt = project_point([-0.5 * (1.0+0.7) + 0.05, 0.0, 0.1250])
      ...
      pt2 = project_point([-0.5 * (1.0+0.7), 0.0, 0.0])
      ...
      pt = project_point([-0.25 * (1.0+0.7), 0.0, 0.05])
      ...
      # draw the induced torque vector
      #
      pt1 = project_point([ 0.0, 0.0,  0.5])
      ...
      pt = project_point([0.0, 0.25, 0.6])

      t.show_text('at'   => pt,
                  'text' => '$\vec{\tau}_{\text{b}} (i_{\text{b}})$')
  ...
  def ia_diagram
    # viewing angles
    #
    alpha = 0.1 * PI            # rotation about y-axis
    beta  = 0.75 * PI / 6              # rotation about z-axis
    dist  = 3.0                 # camera distance in units of focal length

    @zoom  = 1.45 * dist
    @shift = [-0.6, 0.0, 0.0]  # center of image
    ...
    t.show_plot([-1.0/@zoom, 1.0/@zoom, 1.0/@zoom, -1.0/@zoom]) do
      ...
      pt = project_point([0.0, -0.05, 0.125])
      ...
      pt1 = project_point([-0.5 * 0.5*(1.0+0.7 - (1.0-0.7)),
                           0.5 * sqrt(1.0+0.7) * sqrt(1.0-0.7),
                           0.0])

      pt2 = project_point([-0.5 * 0.5*(1.0+0.7 - (1.0-0.7)),
                           0.5 * sqrt(1.0+0.7) * sqrt(1.0-0.7),
                           -0.25])
      ...
      pt = project_point([-0.5 * 0.5*(1.0+0.7 - (1.0-0.7)),
                          0.5 * sqrt(1.0+0.7) * sqrt(1.0-0.7) + 0.05,
                          -0.125])
      ...
      pt = project_point([0.0, 0.125, 0.45])
      ...
      # draw the induced torque vector
      pt1 = project_point([0.0,
                           0.0,
                           0.5])

      pt2 = project_point([-0.25,
                           0.0,
                           0.5])
      ...
      pt = project_point([-0.125,
                          0.0,
                          0.55])

      t.show_text('at' => pt,
                  'text' => '$\vec{\tau}_{\text{a}} (i_{\text{a}})$')
